        ...
        <!-- <draggable v-model="todo" group="tasks"> -->
          <div v-for="todo in todo" :key="todo.id">
            ...
            <div class="bg-white px-3 py-3 mt-3 rounded">
              <a href="#" @click.prevent="deleteTask(todo.id)" class="p-1 mx-1 text-red-600 transform hover:scale-125 motion-reduce:transform-none rounded focus:outline-none focus:ring-2 ring-red-300 ring-offset-2 float-right fas fa-trash-alt"></a>
              ...
              <!-- <p class="mb-3">{{todo}}</p> -->
              <p class="mb-3">{{todo.title}}</p>
              <p class="text-gray-400">By: {{todo.User.name}}</p>
              <p class="text-gray-400">{{ formateDate(todo.createdAt) }}</p>
              <div class="relative">
                <a href="#" @click.prevent="moveCategoryLeft(todo.id)" class="p-1 mx-1 text-gray-600 transform hover:scale-125 motion-reduce:transform-none rounded focus:outline-none focus:ring-2 ring-gray-300 ring-offset-2 absolute bottom-0 right-8 fas fa-arrow-left"></a>
                <a href="#" @click.prevent="moveCategoryRight(todo.id)" class="p-1 mx-1 text-gray-600 transform hover:scale-125 motion-reduce:transform-none rounded focus:outline-none focus:ring-2 ring-gray-300 ring-offset-2 absolute bottom-0 right-0 fas fa-arrow-right"></a>
            </div>
            </div>
          </div>
        <!-- </draggable> -->
