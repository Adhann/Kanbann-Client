<template>
    <div class="flex flex-col box-border mx-5 mb-5 w-full lg:w-72 md:w-72 sm:w-full rounded-lg overflow-hidden items-center ">
      <!-- card-header -->
      <div class="bg-green-600 justify-between w-full px-4 flex py-1 h-10 items-center">
        <p class="text-white">Doing</p>
      ...
      <div class="bg-gray-200 flex flex-col w-full px-3 pb-3 overflow-y-auto" style="max-height: 32rem; min-height: 32rem;">
        <div v-for="doing in doing" :key="doing.id">
          <!-- content board -->
          <div class="bg-white px-3 py-3 mt-3 rounded">
            <a href="#" @click.prevent="deleteTask(doing.id)" class="p-1 mx-1 text-red-600 transform hover:scale-125 motion-reduce:transform-none rounded focus:outline-none focus:ring-2 ring-red-300 ring-offset-2 float-right fas fa-trash-alt"></a>
            ...
            <!-- <p class="mb-3">{{doing}}</p> -->
            <p class="mb-3">{{doing.title}}</p>
            <p class="text-gray-400">By: {{doing.User.name}}</p>
            <p class="text-gray-400">{{ formateDate(doing.createdAt) }}</p>
            <div class="relative">
              <a href="#" @click.prevent="moveCategoryLeft(doing.id)" class="p-1 mx-1 text-gray-600 transform hover:scale-125 motion-reduce:transform-none rounded focus:outline-none focus:ring-2 ring-gray-300 ring-offset-2 absolute bottom-0 right-8 fas fa-arrow-left"></a>
              <a href="#" @click.prevent="moveCategoryRight(doing.id)" class="p-1 mx-1 text-gray-600 transform hover:scale-125 motion-reduce:transform-none rounded focus:outline-none focus:ring-2 ring-gray-300 ring-offset-2 absolute bottom-0 right-0 fas fa-arrow-right"></a>
            </div>
          </div>
